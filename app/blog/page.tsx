        ...
        {featuredPosts && featuredPosts.length > 0 && (
          <section className="mb-16">
            ...
            <div className="grid md:grid-cols-2 gap-8">
              {featuredPosts.map((post: { _id: string; title: string; slug: { current: string }; excerpt?: string; mainImage?: any; publishedAt: string; author?: { name: string; image?: any }; categories?: Array<{ title: string; slug: { current: string }; color?: string }> }) => ( // eslint-disable-line @typescript-eslint/no-explicit-any
                <PostCard
                  key={post._id}
                  title={post.title}
        ...
        <section>
          ...
          {posts && posts.length > 0 ? (
            <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
              {posts.map((post: { _id: string; title: string; slug: { current: string }; excerpt?: string; mainImage?: any; publishedAt: string; author?: { name: string; image?: any }; categories?: Array<{ title: string; slug: { current: string }; color?: string }> }) => ( // eslint-disable-line @typescript-eslint/no-explicit-any
                <PostCard
                  key={post._id}
                  title={post.title}
                  slug={post.slug.current}
